...
describe('core', () => {
  describe('manifest', () => {
    describe('PackageJsonManifest', () => {
      it('should read project name', async () => {
      ...
      it('should read project private', async () => {
        let manifest = new PackageJsonManifest(
          getMockFilePath('public-package.json')
        )

        expect(await manifest.isPrivate()).toBe(false)

        manifest = new PackageJsonManifest(
      ...
      it('should write project version', async () => {
        const manifestPath = await copyMockFile('public-package.json')
        const manifest = new PackageJsonManifest(manifestPath)
